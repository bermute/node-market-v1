...
function createApiRouter(io) {
  ...
  // 특정 게시글의 약속 현황 조회
  router.get("/posts/:id/appointment",async (req, res) => {
    const appointment =  await getAppointmentByPost(req.params.id);
    if (!appointment) {
  ...
  // 약속 철회 요청 (상대방 동의 대기)
  router.post("/posts/:id/appointment/cancel-request", async (req, res) => {
    const post = await getPostById(req.params.id);
    if (!post) {
      return res.status(404).json({ success: false, message: "게시글이 없습니다." });
    }
  ...
  router.post("/posts/:id/appointment/cancel-confirm", async (req, res) => {
    const post = await getPostById(req.params.id);
    if (!post) {
      return res.status(404).json({ success: false, message: "게시글이 없습니다." });
    }
